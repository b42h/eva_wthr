/* CLP2 packed-asset TOC parser (pure, host-testable).
 * Layout: "CLP2" | u16 count | count × {u8 layer, u8 pool, u8 variant,
...
/* Pack now carries 30 cloud entries (normal/storm/merged/lit pools) + 96
 * sprite entries (bolt/ray/moon/drop/trail + 32 rain + 1 fog + 32 sky) = 126.
 * Cap set to 160 for headroom. The pack builder (tools/cloudgen/genpool.py)
 * currently emits 126; if you add asset variants, keep this above the count
...
static inline bool eva_clp_parse(const uint8_t *pack, size_t pack_size,
                                 eva_clp_toc_t *out)
{
    if (!pack || pack_size < 6 || memcmp(pack, "CLP2", 4) != 0) return false;
